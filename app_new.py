...
def main():
    ...
    if uploaded_file or url:
        try:
            ...
            target_language = st.selectbox("Translate to", ["Hindi", "Sanskrit", "Punjabi", "Marathi", "Russian", "French", "Chinese"])
            translator = Translator()
            if st.button("Translate"):
                translated_text = translator.translate(editable_text, dest=target_language.lower()).text
...
'''
            ...
            if st.button("Translate"):
                translated_text = translator.translate(editable_text)
                st.text_area("Translated Text", value=translated_text, height=300)
